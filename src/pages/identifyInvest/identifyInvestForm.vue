  ...
  <div class="identify_wrapper">
    <div class="input_wrapper">
      <div class="form_group">
        <span><img src="../../assets/login/realname.png"></span>
        <input type="text" placeholder="请输入您的姓名" v-model="realName">
      </div>
      <div class="form_group">
        <span><img src="../../assets/login/idcard_no.png" alt=""></span>
    ...
    <div class="choose_box">
      ...
      <dl>
        ...
        <dd @click="chooseFn($event)" data-attr="4">不符合上诉任何一个条件(只可浏览项目,不可投资)</dd>
      </dl>
    </div>
    ...
    <div class="agreement">
      <label class="" >
        ...
        我已阅读<a href="javascript:;" @click="slideToggle()">《相关协议》</a>且理解上述内容，并承担相
      </label>
      <!--<router-link :to="{path:'/riskInfo'}">《风险揭示书》</router-link>和<router-link :to="{path:'/userProtocol'}">《用户服务协议》</router-link>-->
    </div>

    <div class="submit">
      <button :class="{btns_submit:state}" @click="adminInfo();">认证并测试</button>
    </div>
  ...
  export default {
    data() {
      return {
          ...
          isAgree:true,
          actions:[
            {method:this.routerFn(),name:'业务管理规则'},
            {method:this.routerFn(),name:'投资人规则'},
            {method:this.routerFn(),name:'授权协议'},
            {method:this.routerFn(),name:'合格投资人声明承诺'},
            {method:this.routerFn(),name:'风险揭示书'},
          ],
          sheetVisible:false
      };
    },
