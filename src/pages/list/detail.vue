<template>
    <div id='proInfo_container'>
      ...
      <Item :itemList="productInfo" :type="false"></Item>
        ...
        <ul class="btn_nav_wrapper"  id="btn_nav_wrapper" v-show="isVerify || showProjectFlag != 1">
        ...
        <div class="btn_intr_content content" v-if="isShow === 'intr'" style="padding-bottom: 2.2rem;">
          <pro-info :productIntroduceList="productIntroduceList" ></pro-info>
        </div>
        <div class="btn_info_content content" v-else-if="isShow === 'info'">
          <pro-information :productTrading="productTrading"></pro-information>
        </div>
        <div class="btn_progress_content content" v-else-if="isShow === 'progress'">
          <productprogress :productProgressList="productProgressList" ></productprogress>
        </div>
        <div class="btn_procast_content content" v-else>
          <pro-cast :productId="productId"></pro-cast>
        </div>

        <div id="buy_nav" v-if="showProjectFlag != 1">
          <a href="javascript:;" @click="goView();" class="buy_btn" v-if="isVerify || isRisk" style="width: 100%;"><!--没有完成合格投资人认证-->
            认证合格投资人查看更多信息
          </a>

          <a class="yr_tip" v-if="state == '0'"><!--预热期显示开始时间-->
            {{ issBgnDt}} 起售
          </a>

          <div v-if="state == '1' || state == '2'"><!--认购期可以购买项目-->
            <a href="javascript:;" class="tips_buy">{{minPayAmt}}万起认购 {{addAmt}}万累加</a>
