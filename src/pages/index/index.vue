    ...
    <div id="index">
      <div id="swiper-container" v-if="count" class="bannerSwiper swiper-container" >
        <div class="swiper-wrapper">
          <div class="swiper-slide" v-for="item in bannerList" v-if="item.bannerAppUrl"><img :src="item.bannerAppUrl" style="width:100%;"></div>
        </div>
        <!-- 如果需要分页器 -->
      ...
      <div id="tip_wrapper" v-show="homeNoticeList.length">
        <a href="###">
          <router-link :to="{ path:'/tip'}"><span class="icon-tip"></span></router-link>
          <div id="tip_container">
            <div class="tipTitle"></div>
            <div class="swiper-container tipSwiper">
              <div class="swiper-wrapper">
                <div class="swiper-slide" v-for="result in homeNoticeList"> <router-link :to="{ path:'/tipDetail',query:{'noticeId':result.noticeId}}">{{ result.noticeTitle }}</router-link></div>
              </div>
            </div>
          </div>
        </a>
        ...
        projectGet().then(res => {
          if(res.code === ERR_OK){
            if(res.productList == 0){
              ...
              this.items = res.productList.concat(res.productGoodList);
              this.isEmpty = false;
            }
          }
        }).then(res =>{
