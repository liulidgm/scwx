  ...
  <div class="edit_tel_wrapper">
    <div class="content">
      ...
      <div class="form-group group" style="margin-top: 3.45rem">
        <input type="text" v-model.trim="oldCode" placeholder="请输入验证码" @keyup="verify_old_code()" >
        <button @click="getCodeOld($event)">获取验证码</button>
        <span class="errorMsg" v-if="oldCodeError" style="right: 4.5rem;">{{oldCodeError}}</span>
      </div>


      <button class="btn_disabled" @click="submit();" :class="btn_state_change(oldCode)">提交</button>
    </div>
  </div>
</template>
