...
//        6230580000044702697
      ...
      bankCardConfirm(){
        ...
        registNextConfirm(Encrypt(this.buypwd.join('')),this.verifycode).then(res=>{//此处添加银行卡，不需要支付密码，实名绑卡需要
            if(res.code=="000"){
              /*判断绑定的银行卡是不是两步式签约*/
              if( res.isSign == 0){
                /*不需要签约*/
                if(this.oidTenderId){
                  /*支付订单*/
                  this.$router.push({path:'/payOrder?oidTenderId='+this.oidTenderId+'&tipCardFlag=1',query:{title:'支付订单'}})
                }else{
                  /*从个人中心进入跳转回银行卡列表*/
...
.icon_user{background:url('../../assets/login/realname.png');position: absolute;height: 23px;width: 20px;top: 0.65rem;background-size: 20px;}
.icon_idcard{background:url('../../assets/login/idcard_no.png');position: absolute;height: 23px;width: 20px;top: 0.65rem;background-size: 20px;}
